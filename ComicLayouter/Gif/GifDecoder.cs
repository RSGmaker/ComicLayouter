...
namespace Gif.Components
{
	public class GifDecoder 
	{
		...
		{
            byte[] grab = grabbitmapBytes(bitmap);
            int[] pixels = new int[grab.Length];
        ...
        public static byte[] grabbitmapBytes(Bitmap image)
        {
            ...
            for (int x = 0; x < bmpData.Stride; x++)
            {
                ...
                {
                    if (x < bmp.Width && I + 2 < pixels.Length)
                    {
                        pixels[I++] = rgbValues[j++];
                        pixels[I++] = rgbValues[j++];
                        pixels[I++] = rgbValues[j++];
                    }
                    else if (j + 2 < bytes)
        ...
        public static void setbitmapBytes(Bitmap image,byte[] pixels)
        {
            // Create a new bitmap.
            //Bitmap bmp = image.Clone(new Rectangle(Point.Empty, image.Size), System.Drawing.Imaging.PixelFormat.Format24bppRgb);
            Bitmap bmp = image;
		...
		void SetPixels( int [] pixels )
		{
            ...
            for (int th = 0; th < image.Height; th++)
            {
                for (int tw = 0; tw < image.Width; tw++)
                {
                    Color C = Color.FromArgb(pixels[count++]);
